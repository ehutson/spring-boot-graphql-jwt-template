...
describe('useAuth Hook', () => {
    ...
    const { login, logout, getCurrentUser, clearErrors } = jest.requireMock('@/features/auth/store/authSlice.ts');

    // Create a test store for the hook
    const createTestStore = (initialState: Partial<AuthState> = {}) => {
        return configureStore({
            reducer: {
                auth: jest.requireMock('@/features/auth/store/authSlice.ts').default
            },
            preloadedState: {
                auth: {
                    user: null,
